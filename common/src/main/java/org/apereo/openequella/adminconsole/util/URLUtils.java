...
public class URLUtils {
  ...
  public static String getParameterString(Map<?, ?> map) {
    List<Pair<String, String>> nvs = new ArrayList<Pair<String, String>>();
    for (Map.Entry<?, ?> entry : map.entrySet()) {
      ...
      for (Object name2 : values) {
        ...
        nvs.add(new Pair<String, String>(entry.getKey().toString(), element));
      }
    }
    ...
    for (Pair<String, String> nv : nvs) {
      ...
      parameters.append(URLUtils.basicUrlEncode(nv.getFirst()));
      parameters.append('=');
      parameters.append(URLUtils.basicUrlEncode(nv.getSecond()));
    }
  ...
  public static String[] decompose(String url) {
    ...
    if (questionIndex >= 0) {
      bits[0] = StringUtils.safeSubstring(url, 0, questionIndex);
      bits[1] = StringUtils.safeSubstring(url, questionIndex + 1);
    } else {
      bits[0] = url;
      bits[1] = "";
    }
